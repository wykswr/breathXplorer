...
from .cluster import cluster_mz, drop_zero_near
from .find_peak import time_with_peak


def purify(mz: np.ndarray, ints: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    """
...
def scan_mz(file: str, line: bool) -> np.ndarray:
    """
    ...
    with mzml.read(file) as handle:
        all_mz = reduce(concat_func, [sp['m/z array'] for sp in handle if sp['ms level'] == 1]) if line else \
            reduce(concat_func,
                   [purify(sp['m/z array'], sp['intensity array'])[0] for sp in handle if sp['ms level'] == 1])
    return np.unique(all_mz)


def scan_time(file: str) -> np.ndarray:
    """
...
def scan_tic(file: str) -> np.ndarray:
    """
...
def read_sparse(file: str, line: bool) -> pd.DataFrame:
    ...
    with mzml.read(file) as handle:
        for sp in handle:
            if sp['ms level'] == 2:
                continue
            mz_arr, ints_arr = (sp['m/z array'], sp['intensity array']) if line else purify(sp['m/z array'],
                                                                                            sp['intensity array'])
...
def cluster_ms(ms: str, line: bool, dense: float, method: str, n_peak=1) -> dict:
    ...
    for c in np.unique(labels[labels != -1]):
        ...
        records.append(drop_zero_near(tb.loc[np.unique(this_mz)].values, this_mz.mean(), np.unique(this_mz)))
